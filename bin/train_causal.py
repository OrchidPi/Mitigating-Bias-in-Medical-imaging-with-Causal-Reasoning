...
torch.cuda.manual_seed_all(618)
...
from model.classifier_main_new import CombinedClassifier  # noqa
...
def get_loss_causal(output, target, index, cfg):
    ...
    if cfg.criterion_causal == 'CE':
        ...
        #            output[index].view(-1), target, pos_weight=weight)
                ...
                #print(f"target:{target}")
        #else:
        #print(f"target:{target_.shape}")
       # print(f"target:{target_[0]}")
       # print(f"output:{output[index].shape}")
...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
    ...
    bias_header = dataloader.dataset._causal_header
   ...
   # total_ones_per_task = [0] * num_tasks
    for step in range(steps):
        image, target, causal_attr = next(dataiter)
        image = image.to(device)
        target = target.to(device)
        causal_attr = causal_attr.to(device)

        causal_output, causal_output_map, output, logit_map = model(image)
        #print(f'causal_output0:{causal_output[0]}')
        #print(f'causal_output0:{causal_output[0].shape}')
        #print(f'causal_output:{len(causal_output)}')
        #print(f'output0:{output[0]}')
        #print(f'output0:{output[0].shape}')
        #print(f'output:{len(output)}')
        #print(f"causal_attr:{causal_attr.shape}")
        #print(f"causal_attr:{causal_attr[0]}")
       ...
       # optimizer.step()
        ...
        if summary['step'] % cfg.log_every == 0:
            ...
            logging.info(
                '{}, Train, Epoch : {}, Step : {}, Loss : {},  Mainloss:{}, Causalloss:{},'
                'MainAcc : {}, CausalAcc : {}, Run Time : {:.2f} sec'
                .format(time.strftime("%Y-%m-%d %H:%M:%S"),
                        summary['epoch'] + 1, summary['step'], loss_str, loss_main, loss_causal,
            ...
            for i in range(num_causal):
                summary_writer.add_scalar(
                    'train/loss_{}'.format(bias_header[i]), causalloss_sum[i],
                    summary['step'])
        ...
        if summary['step'] % cfg.test_every == 0:
            ...
            logging.info(
                '{}, Dev, Step : {}, Loss : {}, Acc : {}, Auc : {}, Auc_causal:{},'
...
def run(args):
    ...
    #model = CombinedClassifier(cfg)
    model = CombinedClassifier(cfg).to(device)
    if args.verbose is True:
    ...
    if args.pre_train is not None:
        if os.path.exists(args.pre_train):
            ...
            model.module.adapter = torch.nn.Sequential(
                torch.nn.Conv2d(2048, 2050, kernel_size=1),  # Adapting from 2048 to 2050 channels
                torch.nn.BatchNorm2d(2050),
                torch.nn.ReLU(inplace=True)  # Optional but can help in stabilizing the learning
                ).to(device) 

            #model.module.load_state_dict(ckpt, strict=False)
            model.classifier = torch.nn.Identity()
            model.fc_0 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))
            model.fc_1 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))
            model.fc_2 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))
            model.fc_3 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))
            model.fc_causal_1 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))
            model.fc_causal_2 = torch.nn.Conv2d(2050, 1, kernel_size=(1, 1), stride=(1, 1))

            model.bn_0 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
            model.bn_1 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
            model.bn_2 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
            model.bn_3 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
            model.bn_causal_1 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
            model.bn_causal_2 = torch.nn.BatchNorm2d(2050, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
    ...
    for epoch in range(epoch_start, cfg.epoch):
        ...
        summary_train, best_dict = train_epoch(
            ...
            summary_writer, best_dict, dev_header, dev_causal_header)
